...
#if IS_ENABLED(CONFIG_ZMK_DONGLE_DISPLAY_BONGO_CAT)
    lv_obj_align_to(zmk_widget_device_name_obj(&device_name_widget), zmk_widget_bongo_cat_obj(&bongo_cat_widget), LV_ALIGN_OUT_TOP_LEFT, PADDING_LEFT, -2);
#else
...
#else
    lv_obj_align(zmk_widget_hid_indicators_obj(&hid_indicators_widget), LV_ALIGN_BOTTOM_LEFT, PADDING_LEFT, 0);
#endif
#endif

    zmk_widget_layer_status_init(&layer_status_widget, screen);
#if IS_ENABLED(CONFIG_ZMK_DONGLE_DISPLAY_MODIFIERS)
    lv_obj_align_to(zmk_widget_layer_status_obj(&layer_status_widget), zmk_widget_modifiers_obj(&modifiers_widget), LV_ALIGN_OUT_TOP_LEFT, PADDING_LEFT, -2);
#else
